...
impl App {
            ...
            InputMode::Command => {
                match key.code {
                    ...
                    KeyCode::Char(c) => {
                        ...
                        self.input_cursor += 1;
                        self.command_index = 0; // Reset selection on new input
                    }
                    _ => {}
                }
            }
    ...
    async fn handle_nodes_key(&mut self, key: KeyEvent) -> bool {
        // Handle EditSource mode first - it intercepts all input
    ...
    async fn execute_command(&mut self) {
        ...
        // If showing suggestions and input isn't an exact command, use the selected suggestion
    ...
    pub async fn tick(&mut self) -> Result<()> {
        ...
        // Refresh status periodically (every 250ms for real-time feel)
    ...
    async fn refresh_status(&mut self) -> Result<()> {
        ...
        // Refresh nodes from daemon
        if let Some(client) = &self.daemon_client {
            if let Ok(nodes) = client.list_nodes().await {
                // Only update if we got nodes, or if we had no nodes before
                // This prevents flickering when daemon temporarily returns empty list
                if !nodes.is_empty() || self.nodes.is_empty() {
                    self.nodes = nodes;
                    self.daemon_available = true;
                }
            } else {
                // Query failed - daemon might be unavailable
                self.daemon_available = false;
            }
        }

        // Refresh discoverable nodes and sources based on view
        ...
        // Refresh logs if viewing node logs
        if let View::NodeLogs(ref node_name) = self.view {
            let name = node_name.clone();
            self.fetch_node_logs(&name).await;
        }

        // Refresh service status in NodeDetail view
    ...
    /// Fetch systemd service status for a node
            ...
            if let Some(client) = &self.daemon_client {
                match client.add_node(&path).await {
                    Ok(_) => {
                        ...
                        // Immediately add to installed nodes for instant feedback
                        ...
                        // Also refresh from daemon to get accurate status
                        let _ = self.refresh_status().await;
                    }
                    Err(e) => self.add_message(format!("Error: {}", e), MessageType::Error),
                }
    ...
    /// Handle key events in EditSource mode (marketplace form)
    fn handle_edit_source_key(&mut self, key: KeyEvent) -> bool {
        match key.code {
            ...
            KeyCode::Tab => {
                ...
                self.marketplace_active_field = if self.marketplace_active_field == 0 { 1 } else { 0 };
            }
            KeyCode::Enter => {
                // Save the source
    ...
    /// Save marketplace source (add or update)
    fn save_marketplace_source(&mut self) {
        ...
        if path.contains("github.com") || path.starts_with("git@") || path.ends_with(".git") {
            self.add_message("Git sources coming soon! Use local paths for now.".into(), MessageType::Warning);
            return;
        }
        ...
        let result = if let Some(ref original_path) = self.marketplace_edit_path {
            ...
            self.registry.update_source(original_path, &name, &expanded_path)
        } else {
            // Add mode - create new
        ...
        match result {
            Ok(()) => {
                let action = if self.marketplace_edit_path.is_some() { "Updated" } else { "Added" };
