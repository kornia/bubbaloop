...
impl<P: PlatformOperations> BubbaLoopMcpServer<P> {
    ...
    async fn register_alert(
        ...
        // Validate BEFORE dispatching to the backend so mock and daemon
        // platforms reject identical inputs identically. We reconstruct
        // a ReactiveRuleConfig with the same defaults the backend will
        // apply (see crate::daemon::reactive::DEFAULT_*) so the check
        // sees the exact values that will ultimately be persisted.
        {
            use crate::daemon::reactive::{
                ReactiveRuleConfig, DEFAULT_AROUSAL_BOOST, DEFAULT_DEBOUNCE_SECS,
            };
            let preview = ReactiveRuleConfig {
                // Placeholder id — the real one is generated by the
                // backend. `validate` only requires it to be non-empty.
                id: "preview".to_string(),
                mission_id: req.mission_id.clone(),
                predicate: req.predicate.clone(),
                debounce_secs: req.debounce_secs.unwrap_or(DEFAULT_DEBOUNCE_SECS),
                arousal_boost: req.arousal_boost.unwrap_or(DEFAULT_AROUSAL_BOOST),
